...
description: >-
  Sync boilerplate for this cookiecutter template (cookiecutter-docker-image,
  Docker image) from reference repos using only origin/main;
...
**This template:** `cookiecutter-docker-image` — Docker image cookiecutter.
...
- Port from reference repos only what fits **this** tier (Docker image).
...
|------|--------------|--------|
| Meta | `cookiecutter-cookiecutter` | Bakes child templates; sync docs live under baked `cookiecutter-docker-image/` |
| Language/tool | `cookiecutter-ruby` | Docker image boilerplate |
...
- Cookiecutter vars: `cookiecutter-docker-image`, `Docker image`
